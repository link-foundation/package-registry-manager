...
fn documentation_deploy_uses_github_pages_artifact_flow() {
    ...
    assert!(deploy_docs.contains("path: rust/target/doc"));
    ...
    assert!(!deploy_docs.contains("publish_dir: rust/target/doc"));
}
...
fn documentation_deploy_publishes_working_pages_root() {
    ...
    assert!(deploy_docs.contains("cargo metadata --manifest-path rust/Cargo.toml"));
    assert!(
        deploy_docs.contains(r#"replace("-","_")"#),
        "root redirect should use rustdoc's crate directory naming"
    );
    assert!(
        deploy_docs.contains("rust/target/doc/index.html"),
    ...
    assert!(
        deploy_docs.contains("rust/target/doc/.nojekyll"),
    ...
    assert!(
        deploy_docs.contains("find rust/target/doc -maxdepth 2 -print"),
...
fn cargo_lock_guard_blocks_cached_cargo_jobs() {
    ...
    assert!(
        cargo_lock.contains("rust-script rust/scripts/check-cargo-lock.rs"),
...
fn release_jobs_smoke_test_published_crate_before_release_artifacts() {
    ...
    for job_name in ["auto-release", "manual-release"] {
        ...
        assert!(
            job.contains("rust-script rust/scripts/smoke-test-published-crate.rs"),
...
fn release_jobs_check_crate_size_before_publishing() {
    ...
    for job_name in ["auto-release", "manual-release"] {
        ...
        assert!(
            job.contains("rust-script rust/scripts/check-crate-size.rs"),
...
fn build_job_checks_crate_size() {
    ...
    assert!(
        build.contains("rust-script rust/scripts/check-crate-size.rs"),
...
fn rust_script_is_installed_through_the_retrying_locked_helper() {
    ...
    assert!(
        workflow.contains("run: ./rust/scripts/install-rust-script.sh"),
...
fn rustdoc_warnings_are_gated_before_release_jobs() {
    ...
    assert!(lint_docs.contains("--manifest-path rust/Cargo.toml"));
}
...
fn documentation_deploy_keeps_fail_closed_rustdoc_flags() {
    ...
    assert!(build_docs.contains("--manifest-path rust/Cargo.toml"));
}
