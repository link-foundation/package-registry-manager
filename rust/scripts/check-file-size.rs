...
const LEGACY_SOURCE_LIMITS: &[(&str, usize)] = &[
    ("rust/scripts/version-and-commit.rs", 1332),
    ("rust/tests/unit/ci-cd/workflow_release.rs", 1048),
];
...
mod tests {
    ...
    fn existing_size_debt_cannot_grow_and_generated_history_is_excluded() {
        assert_eq!(
            limits_for("rust/scripts/version-and-commit.rs"),
            (1232, 1332)
        );
        assert_eq!(
            limits_for("rust/tests/unit/ci-cd/workflow_release.rs"),
            (948, 1048)
        );
