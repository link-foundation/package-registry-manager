...
//! Bump and commit a release from either a root crate or `rust/` package.
//! crates.io, rather than Git tags, is the source of published versions.
...
// Test harnesses do not enter `main`, so its otherwise reachable helpers look unused.
...
fn main() {
    ...
    let js_synced = Path::new("js/package.json").exists();
    if js_synced {
        let sync_script = Path::new(&rust_root).join("scripts/sync-js-version.rs");
        let sync_script = sync_script.to_string_lossy().to_string();
        if let Err(e) = exec("rust-script", &[&sync_script, &new_version]) {
            eprintln!("Error synchronizing the JavaScript package version: {}", e);
            exit(1);
        }
    }
    ...
    if js_synced {
        add_args.extend(["js/package.json", "js/package-lock.json"]);
    }
